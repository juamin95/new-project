...
**Projekt-Anlegen (2026-07-23) — Berechtigungs-Klarstellung [[berechtigungen-entwurf-first]].** Julian stellte klar: der Agent DARF anlegen (nicht nur lesen); Schutz = Draft-first + Mensch veröffentlicht/versendet. Erste Umsetzung: **Projekt anlegen**. Agent-Werkzeug `projekt_vorschlagen` (löst Kunde + innere `address.id` via `kontakt suchen` selbst auf, wählt Gewerk/Projekttyp) → zentriertes Pop-up (Ablehnen · Anpassen · Bestätigen) → gated Schreib-Op `projekt-anlegen` (`hero projekt anlegen`) über `POST /api/conversations/[id]/termin`-Muster (`.../projekt`), Audit in `cockpit_actions` (type erweitert per Migration auf termin/projekt/aufgabe/checkliste/dokument). Governance-Regel `.claude/rules/hero-tools.md` entsprechend umgestellt. **Verifiziert (Vorschlag):** „Erstelle ein Projekt für Julian Amini, Gartenpflege, Bergstraße 12" → korrekter Vorschlag (Kunde 5711737, address.id 15737567, gewerk gartenpflege); bei mehreren Adressen fragt der Agent nach. Echter Schreib-Test steht noch aus (Projekte sind per API nicht löschbar, nur archivierbar) — erster echter „Bestätigen" legt ein reales Projekt an. Integrationstest 401/400/200/502.
