...
- **Zuordnungs-Pop-up (`conversation.tsx`):** Kommt vom Agenten ein Vorschlag zurück, öffnet ein **zentriertes Pop-up** (shadcn `Dialog`) mit Projekt/Kunde + Titel. (Bewusst Dialog statt Bottom-Sheet — mittig, nicht gequetscht neben der Navigationsleiste; dieselbe Bestätigungs-Komponente ist die Basis für die geplante universelle Aktions-Vorschau, u. a. Termin genehmigen.) **Bestätigen** → `PATCH /conversations/[id]` (Titel/Fortschritt sofort aktualisiert); **Ablehnen** verwirft. Nichts wird ohne Bestätigung verknüpft.
